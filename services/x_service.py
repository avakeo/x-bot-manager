...
def send_tweet_with_media(account, text, image_names=None):
    ...
    # v1.1 API（画像アップロード用）
    auth_v1 = tweepy.OAuth1UserHandler(auth_info["ck"], auth_info["cs"], auth_info["at"], auth_info["as"])
    api_v1 = tweepy.API(auth_v1)

    # v2 API（ツイート投稿用）
    client_v2 = tweepy.Client(
        consumer_key=auth_info["ck"], consumer_secret=auth_info["cs"],
        access_token=auth_info["at"], access_token_secret=auth_info["as"]
    )
    ...
    if not text and not media_ids:
        raise ValueError("テキストまたは画像が必要です（画像ファイルが見つからないか、アップロードに失敗しました）")
